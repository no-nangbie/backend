package com.nonangbie.S3.service;
...
public class S3Uploader {
    ...
    private String putS3(File uploadFile, String fileName) {try {
        amazonS3.putObject(new PutObjectRequest(bucket, fileName, uploadFile)
                .withCannedAcl(CannedAccessControlList.PublicRead));
    } catch (AmazonServiceException e) {
        // S3에서 발생한 서비스 예외 처리 (권한 문제, 버킷 문제 등)
        System.err.println(e.getErrorMessage());
    } catch (SdkClientException e) {
        // 네트워크 문제 등 클라이언트 쪽 예외 처리
    ...
    public String getFileUrl(String fileName) {
        try {
            // URL 디코딩을 통해 원래의 파일 이름을 가져옵니다.
            String decodedFileName = URLDecoder.decode(fileName, "UTF-8");
            return amazonS3.getUrl(bucket, decodedFileName).toString();  // S3에서 해당 파일의 URL 가져오기
        } catch (UnsupportedEncodingException e) {
            log.error("Error while decoding the file name: {}", e.getMessage());
